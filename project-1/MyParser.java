...
class MyParser {
	...
	/*
	 * Returns the time (in YYYY-MM-DD HH:MM:SS format) denoted by a time string
	 * like Dec-31-01 23:59:59.
	 */
	...
	static void generateUserLoadFile(Element root) {
		try {
			...
			for (Element itemElem : itemsElems) {
				...
				output.print(appendSeparator(getElementTextByTagNameNR(
						itemElem, "Location")));
				...
				for (Element bidElem : bidElems) {
					Element bidderElem = getElementByTagNameNR(bidElem,
							"Bidder");
					output.print(appendSeparator(bidderElem
							.getAttribute("UserID")));
					output.print(appendSeparator(bidderElem
							.getAttribute("Rating")));
					output.print(appendSeparator(getElementTextByTagNameNR(
							bidderElem, "Location")));
					output.println(getElementTextByTagNameNR(bidderElem,
							"Country"));
				}
	...
	static void generateItemLoadFile(Element root) {
		try {
			...
			for (Element itemElem : itemsElems) {
				...
				output.print(appendSeparator(getElementTextByTagNameNR(
						itemElem, "Name")));
				output.print(appendSeparator(formatDollar(getElementTextByTagNameNR(
						itemElem, "Currently"))));
				output.print(appendSeparator(getStandInNullWhenEmpty(formatDollar(getElementTextByTagNameNR(
						itemElem, "Buy_Price")))));
				output.print(appendSeparator(formatDollar(getElementTextByTagNameNR(
						itemElem, "First_Bid"))));
				output.print(appendSeparator(formatTime(getElementTextByTagNameNR(
						itemElem, "Started"))));
				output.print(appendSeparator(formatTime(getElementTextByTagNameNR(
						itemElem, "Ends"))));
				...
				Element sellerElem = getElementByTagNameNR(itemElem, "Seller");
				output.print(appendSeparator(sellerElem.getAttribute("UserID")));
				output.println(getElementTextByTagNameNR(itemElem,
						"Description"));
			}
	...
	static void generateCategoryLoadFile(Element rootElem) {
		try {
			...
			for (Element itemElem : itemElems) {
				Element[] categoryElems = getElementsByTagNameNR(itemElem,
						"Category");
				for (Element categoryElem : categoryElems) {
					output.print(appendSeparator(itemElem
							.getAttribute("ItemID")));
		...
		try {
			...
			for (Element itemElem : itemElems) {
				...
				for (Element bidElem : bidElems) {
					...
					output.print(appendSeparator(formatTime(getElementTextByTagNameNR(
							bidElem, "Time"))));
					output.println(formatDollar(getElementTextByTagNameNR(
							bidElem, "Amount")));
				}
			}
